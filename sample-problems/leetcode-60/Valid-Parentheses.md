...
**1. Overall Structure.** A *valid prefix* of `s` with closing index
`i` is a substring `s.(lob..i)` which is valid, i.e. the opening
brackets and closing brackets are balanced correctly. We can use this
definition to implement a solution with the following structure:
...
**2.** We introduce the variable `valid-end` to denote the index of
the last character of the longest valid prefix of `s.(lob..i)`. At the
end, when we've scanned the entire string, `valid-end` should denote
the longest valid prefix of `s.(lob..hib)`. Then we can use it to
compute `valid`:

    loc.{1} valid-end
    {1end}: valid var bool := (valid-end = s.hib)

**3.** It might not make sense to scan the entire string if it includes an
...
terminating early:
    ...
    {1find}:
      valid-end var int := s.lob-1 -- an empty prefix of s is valid
      do: {3cond valid-end could still be extended} ->
        {3scan scan one more character of s
               and extend valid-end if possible}

**4.** We say that a prefix of `s` *could be part of a valid prefix* if there
...
conditions that must be satisfied by `i` and `valid-end` respectively:
    ...
    loc.{1,3.do.before} i var int := s.lob-1
    inv.{3.do} P1: "s.(lob..i) could be part of a valid prefix"
    inv.{3.do} P2: "s.(lob..valid-end) is the longest valid prefix of s.(lob..i)"

**5.** To determine when `valid-end` can no longer be extended, we
introduce an additional boolean variable `fin` and an invariant
predicate `P3`. The loop can be exited either when `i = s.hib`
(`valid-end` identifies the longest valid prefix of
`s.(lob..i), i = hib`) or when `fin` is set to true (and `valid-end`
can no longer be extended, and hence identifies the longest valid
prefix of `s`):

    loc.{1,3.do.before} fin var bool := false
    inv.{3.do} P3: fin => "s.(lob..i+1) could not be part of a valid prefix"
    {3cond valid-end could still be extended}: i < s.hib and non fin
...
**6.** The need to update different variables gives us an obvious way
to split the loop body. First we scan an additional character and
extend `i` (satisfying conditions `P1` and `P3`). Then we extend
`valid-end` to identify the longest valid prefix of `s.(lob..i)`:

    {3scan scan one more character of s
           and extend valid-end if possible}:
      {6i check if s.(lob..i+1) could be part of a valid prefix
          and extend i, fin}
      {6valid-end check if s.(lob..i) is a valid prefix
                  and extend valid-end if possible}

**7.** The operation `{6i}` must examine the character at `s.(i+1)`
and, if it's a closing parenthesis, must determine if it can be
matched to an earlier parenthesis in the string. To do this, we
introduce a variable `parens` that keeps track of the currently open
parentheses in `s.(lob..i)`:

    loc.{1,3.do.before} parens var list.int := ()
    inv.{3.do} P4: "parens gives the indices of unclosed parens in s.(lob..i)"

The invariant `P4` makes it easy to fill in `{5valid-end}':

    {6valid-end}: when: parens.empty -> valid-end := i :when

**8.** At this point, the remainder of the program is straightforward to
...
Then proceed by cases to finish the program:
    ...
    wrap.{3.do.body} use s, mod valid-end, i, fin, parens, loc nc, lc
    -- <- creates a local scope around the body of (3.do)
    ins.{3scan.before} nc var char := s.(i+1)
    ins.{3scan.before} lc var maybe.char := maybe.s.(parens.high)
...
<!-- previously had a BUG (non is-closing.nc -> parens:hiext i; i := i + 1) -->
